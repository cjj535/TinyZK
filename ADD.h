...
public:
	...
	Message (EC_GROUP *g) {
		...
		for (int i=0;i<5;i++) this->str_len[i]=0;
...
void prover(EC_GROUP *g, const EC_POINT *G, const EC_POINT *H, Message *message)
{
	...
	BN_mod_add(bn_tmp1, alpha, beta, order, ctx);
	EC_POINT_mul(g,ec_point_tmp2,NULL,H,bn_tmp1,NULL);
	...
	//u=x+(a+b)e
	BN_mod_add(bn_tmp1, prover.a, prover.b, order, ctx);
...
bool verifier_verify(EC_GROUP *g, const EC_POINT *G, const EC_POINT *H, Message *message) {
	...
	//A+B=C?
	EC_POINT_add(g,ec_point_tmp1,A,B,NULL);
	if (EC_POINT_cmp(g,ec_point_tmp1,C,NULL)) return false;
