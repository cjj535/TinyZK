...
void prover(EC_GROUP *g, const EC_POINT *G, const EC_POINT *H, Message *message)
{
	...
	//s = _gamma - alpha*b
	BN_mod_mul(bn_tmp1, alpha, prover.b, order, ctx);
	BN_mod_sub(s, _gamma, bn_tmp1, order, ctx);

	//compute D1=r_b*A+r_s*H
	EC_POINT_mul(g,ec_point_tmp1,NULL,A,r_b,NULL);
...
bool verifier_verify(EC_GROUP *g, const EC_POINT *G, const EC_POINT *H, Message *message) {
	...
	//m_b*A+m_s*H
	EC_POINT_mul(g,ec_point_tmp1,NULL,A,message->m_b,NULL);
